...
def classify_columns(
    ...
    for col in df.columns:
        ...
        # Classification logic
        if dtype in [int, float, 'int64', 'float64', np.int64, np.float64]:
            # Numeric columns → KPI Metrics (highest priority)
            # This prevents high-uniqueness numerics from being classified as IDs
            kpis.append(col)
            logger.debug(f"  {col}: KPI Metric (dtype={dtype})")
        
        elif dtype == 'object':
            # String columns → Text Dimension (second priority)
            # Unless they are ID columns (handled below)
            if is_id_column:
                dimensions_id.append(col)
                logger.debug(f"  {col}: ID Dimension (dtype=object, is_id=True)")
            else:
                dimensions_text.append(col)
                logger.debug(f"  {col}: Text Dimension (dtype={dtype})")
        
        elif is_id_column or unique_ratio > 0.9:
            # Non-numeric with high uniqueness → ID Dimension
            dimensions_id.append(col)
            logger.debug(f"  {col}: ID Dimension (is_id={is_id_column}, unique_ratio={unique_ratio:.1%})")
        
        else:
            # Fallback: classify as text
            dimensions_text.append(col)
            logger.debug(f"  {col}: Text Dimension (default fallback)")
