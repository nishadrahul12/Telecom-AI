...
def classify_columns(
    ...
    for col in df.columns:
        ...
        if dtype in [int, float, np.int64, np.float64]:
            # ✓ Numeric columns (int/float) → KPI Metrics
            # This is FIRST priority - no uniqueness checks
            kpis.append(col)
            logger.debug(f"  {col}: KPI Metric (dtype={dtype})")
        
        elif dtype == 'object':
            # ✓ String columns → check if ID name
            if is_id_column:
                # Text column with 'id' in name → ID Dimension
                dimensions_id.append(col)
                logger.debug(f"  {col}: ID Dimension (dtype=object, is_id=True)")
            else:
                # Regular text column → Text Dimension
            ...
            # (only reaches here if not numeric or object)
            dimensions_id.append(col)
