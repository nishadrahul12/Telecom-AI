...
def classify_columns(
    ...
    for col in df.columns:
        ...
        if dtype == 'object':
            ...
            # ID column OR mostly unique values → ID Dimension
            dimensions_id.append(col)
            logger.debug(f"  {col}: ID Dimension (is_id={is_id_column}, unique_ratio={unique_ratio:.1%})")
        
        elif dtype in [int, float, 'int64', 'float64', np.int64, np.float64]:
            # Numeric → KPI
            kpis.append(col)
            logger.debug(f"  {col}: KPI Metric (dtype={dtype})")
        
        else:
            # Edge case: try to treat as KPI if numeric-like
            try:
                pd.to_numeric(df[col])
                kpis.append(col)
                logger.debug(f"  {col}: KPI Metric (coerced to numeric)")
            except:
                dimensions_text.append(col)
