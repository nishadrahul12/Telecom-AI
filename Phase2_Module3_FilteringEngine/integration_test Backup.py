...
metadata = DataFrameMetadata(
    ...
    kpi_columns=[col for col in df.columns 
                 if col not in ['TIME', 'REGION', 'CITY', 'DISTRICT', 'SITENAME', 
                               'CARRIER_NAME', 'MRBTS_ID', 'LNBTS_ID', 'LNCEL_ID', 
                               'BAND_NUMBER', 'POSTCODE', 'MRBTS_NAME', 'LNBTS_NAME']],
    time_column='TIME',
    data_level='Cell'
)
...
assert elapsed < 0.1, "Performance target missed!"

# TEST 1.3: Multi-dimensional filter
print("\n--- Test 1.3: Multi-Dimensional Filter (REGION + CARRIER_NAME) ---")
start = time.time()
result = apply_filters_and_sample(
    df, metadata, 'Cell', 
    {'REGION': ['N1'], 'CARRIER_NAME': ['L700', 'L1800']}
)
elapsed = time.time() - start
print(f"✅ Filtered: {result.row_count_filtered} rows")
print(f"✅ Sampled: {result.row_count_sampled} rows")
...
print(f"✅ All REGION values correct: {filtered_df['REGION'].unique()}")
print(f"✅ All CARRIER_NAME values correct: {filtered_df['CARRIER_NAME'].unique()}")
print(f"✅ No NaN in key columns: {not filtered_df[['REGION', 'CARRIER_NAME']].isna().any().any()}")
