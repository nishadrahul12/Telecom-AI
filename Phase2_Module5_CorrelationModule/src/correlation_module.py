...
def calculate_correlation_matrix(
    ...
    **Algorithm**:
    ...
    # FIX 1: Column existence check moved to before the minimum 2 KPIs check
    # to allow the specific error for missing columns to fire correctly.
    ...
    # ✓ Minimum 2 KPIs check (MUST be after column existence check)
    ...
    assert np.allclose(correlation_matrix, correlation_matrix.T, rtol=1e-4, atol=1e-7, equal_nan=True), \
        "Correlation matrix is not symmetric"
    
    # ✓ Validation: Check diagonal is 1.0
    diagonal = np.diag(correlation_matrix)
    valid_diagonal_elements = diagonal[~np.isnan(diagonal)]
    assert np.allclose(valid_diagonal_elements, 1.0, rtol=1e-4, atol=1e-7), \
...
def get_top_3_correlations(
    ...
    **Algorithm**:
    ...
    # FIX 2: Added explicit check for minimum 2 KPIs to prevent silent failure
    # and satisfy test_single_kpi_correlation.
    if len(kpi_names) < 2:
        raise ValueError(f"At least 2 KPIs are required to find non-self correlations, got {len(kpi_names)}")
